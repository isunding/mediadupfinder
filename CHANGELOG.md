...
本次更新聚焦「用起来更省心」：运行期文件分目录收纳、配置页每张卡片补上帮助说明、进度页数字与文件名不再抖动，并修正 `web.bat` 抢先打开浏览器导致的 `Failed to fetch`。

### 新增 — 运行时文件分目录

- **`logs/`**：扫描日志 `scan_YYYYMMDD.log` 统一收纳
- **`history/`**：任务历史 `scan_history.json` 统一收纳
- **`config/`**：元数据缓存 `.mdf_cache.json`、单实例锁 `.web.lock` 统一收纳
- **旧文件自动迁移**：首次启动时把散落在项目根目录的 `scan_*.log` / `scan_history.json` / `.mdf_cache.json` / `.web.lock` 自动移入对应目录，无需手动整理

### 新增 — 配置页帮助提示

- **9 处 `?` 帮助圆圈**：配置预设、生成 `web.bat`、扫描范围、基础参数、候选分档、高级阈值、输出选项、采样预览、开始扫描，鼠标悬停或键盘聚焦即显示说明
- **纯 CSS tooltip**：`::after + content:attr(data-tip)` 实现，无额外 JS 依赖；靠右的元素用 `.right` 变体避免溢出视口
- **中英双语**：帮助文案随页面语言切换（`data-i18n-tip`）

### 新增 — 一键启动脚本

- **`一键启动Web.bat`**（Windows）：双击即检查依赖（缺失时自动 `pip install flask flask-cors`）、探测 MediaInfo 与 FFmpeg、启动 Web 服务并延迟打开浏览器，风格与既有 `一键扫描.bat` 保持一致

### 优化

- **进度页固定宽度防抖动**：进度 / 实时速率 / 预计剩余 / 已用时 四张指标卡固定 `160px` 等宽，数值使用 `font-variant-numeric: tabular-nums` 等宽数字，极端数值下不再左右跳动
- **当前文件名换行显示**：状态行拆为两行 —— 第一行"正在解析 I:\ 第 942/20912 个"，第二行显示文件名；两行各预留固定高度（文件名区 40px / 两行截断），短名、长名、无文件名三种情形下页面高度均不变化

### 修复

- **`web.bat` 打开浏览器早于服务就绪**：脚本原先先开浏览器再启动服务，浏览器抢跑导致页面报 `Failed to fetch`；改为后台 `ping` 延迟 2 秒后再打开浏览器

### 测试

- 测试用例 **52 passed**，`flake8 --select=E9,F63,F7,F82` 零告警
